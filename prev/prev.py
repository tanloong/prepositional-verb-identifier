...
class PREV:
    def __init__(
        self, is_pretokenized: bool, is_refresh: bool, is_visualize: bool, print_what: str
    ) -> None:
    ...
    def build_doc_stanza(self, text: str, ifile: str):
        pkl_file = ifile.replace(".txt.tokenized", "") + "_pos-tagged.pkl"
    ...
    def build_doc_spacy(self, text: str, ifile: str):
        ...
        pkl_file = ifile.replace(".txt.tokenized", "") + "_dep-parsed.pkl"
        if not self.is_refresh and os.path.exists(pkl_file):
        ...
        for prep in preps:
            ...
            patterns.append(
                [
                    ...
                    {
                        "LEFT_ID": "verb",
                        "REL_OP": ">+",  # B is a right immediate child of A, i.e., A > B and A.i == B.i -1
                        "RIGHT_ID": "prep",
                        "RIGHT_ATTRS": {"ORTH": prep, "DEP": "prep"},
                    },
                ]
    ...
    def is_in_passive_voice(self, verb_id: int, sent_spacy: Span) -> bool:
        # verbs in passive voice should be excluded
        is_passive = False
        for child in sent_spacy[verb_id].children:
    ...
    def parse_matches(self, matches: List[tuple], pattern: List[dict], sent_spacy: Span) -> str:
        ...
        for match in matches:
            _, token_ids = match
            verb_id = token_ids[0]
            if not self.is_in_passive_voice(verb_id, sent_spacy):
                result_sent += (
                    ", ".join(
